...
本プロジェクト固有の `AGENTS.md`、`docs/policies/`、`docs/specs/`、`docs/tasks/` は、Adlaire Git Repository の運用正本である。これらをそのまま他プロジェクトへ配布してはならない。
...
policy slots は、各プロジェクトで共通方針を維持したまま具体化する2類ポリシーの必須枠である。policy slots は方針を自由に変更する場所ではない。採用技術、固定バージョン、ライセンス、デプロイ方式、データ保存、依存関係、セキュリティ要件は AdlaireGroup 共通方針として扱い、各プロジェクトではプロジェクト名、配置先、サーバ名、ドメイン、仕様書名、タスク管理、実装対象機能などの固有値を具体化する。
...
本プロジェクトの `AGENTS.md`、2類ポリシー、3類マスター仕様書、タスク管理方針、README を変更する場合は、その変更が AdlaireGroup 共通方針に該当するかを必ず判定する。
...
- Git ホスティング固有の画面、API、DB schema、固有実装対象
...
AdlaireGroup 共通ガバナンス雛形は、各プロジェクトの運用正本そのものではない。

| 区分 | 役割 |
|---|---|
| 共通コア | `AGENTS.md`、ドキュメント索引、ドキュメント憲章、2類ポリシー総則の共通構造を提供する |
| policy slots | 共通方針を破らず、各プロジェクトが固有値を具体化するための必須枠を提供する |
| 各プロジェクトの運用正本 | 実際のプロジェクト名、仕様書名、タスク管理、実装対象、配置先、サーバ名を確定する |

共通雛形には、プロジェクト固有の機能仕様、固有タスク、固有配置パス、固有ドメイン、固有サーバ名を直接固定しない。必要な項目は placeholder または policy slots の個別具体化必須項目として定義する。

## 6. 更新判定チェック

共通雛形を更新する場合は、最低限以下を確認する。

- AdlaireGroup 関連プロジェクト、プロダクトへ横展開すべき共通方針であるか。
- プロジェクト固有仕様を共通雛形へ混入していないか。
- 共通コアと policy slots の責務を混同していないか。
- 採用技術、固定バージョン、ライセンス、デプロイ方式、データ保存、依存関係、セキュリティ要件の共通方針と矛盾しないか。
- 承認工程、Pull Request 経由、`main` 直接 push 禁止、読了義務、ドキュメント整合性の共通原則を弱めていないか。

各プロジェクトの文書を更新したが共通雛形を更新しない場合は、その変更がプロジェクト固有である理由を Pull Request 説明に記載する。
