...
| 各プロジェクトの運用正本 | 実際のプロジェクト名、仕様書名、Phase 計画、実装対象、配置先、サーバ名を確定する |

共通雛形には、プロジェクト固有の機能仕様、固有Phase計画、固有配置パス、固有ドメイン、固有サーバ名を直接固定しない。必要な項目は placeholder または policy slots の個別具体化必須項目として定義する。
