...
本書は、`AGENTS.md`、2類ポリシー、3類マスター仕様書、タスク管理ファイルの代替ではない。

作業者は、必ず最初に `AGENTS.md` を読む。その後、本書を索引として、作業に必要な2類ポリシー、3類マスター仕様書、タスク管理ファイル、関連文書を確認する。
...
| `docs/policies/DOCUMENT_CHARTER.md` | ドキュメント憲章。ドキュメント体系、責務境界、タスク管理、整合性改訂方針 |
...
| ドキュメント | 役割 |
|---|---|
| `docs/specs/Auris_System_Design.md` | システム全体のマスター仕様書。全体方針、仕様判断フロー、3類マスター仕様改善基準、現行正本仕様、未確定、対象外、責務分担、SDK 接続一本化、機能ドメイン方針の基準 |
...
3類マスター仕様書は、現行正本仕様、過去履歴、保留候補、対象外範囲、未確定範囲を分離して読む。過去の Phase 表記は履歴として保持し、今後の仕様判断では、各マスター仕様書の現行正本仕様、2類ポリシー、タスク管理ファイルを基準にする。バージョンは開発版 `v.0.N` と安定版 `v.X.Y` を分離して扱う。
...
| ドキュメント | 役割 |
|---|---|
| `docs/tasks/TASK_MANAGEMENT.md` | タスク管理ファイル。未策定、未改訂の策定タスクと未実装の実装タスクを管理する |
...
本プロジェクトの `AGENTS.md`、2類ポリシー、3類マスター仕様書、タスク管理方針を変更する場合は、変更内容が AdlaireGroup 共通方針に該当するかを判定し、該当する場合は `docs/tpl-governance/` も同一変更範囲で更新する。

採用技術、固定バージョン、ライセンス、デプロイ方式、データ保存、依存関係、セキュリティ要件は共通方針として扱う。プロジェクト固有値、仕様書名、タスク管理、実装対象機能は、共通方針を破らない範囲で各プロジェクトが具体化する。
