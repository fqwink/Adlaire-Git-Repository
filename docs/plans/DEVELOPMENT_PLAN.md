...
- `AGENTS.md` の最上位ルール整理
- 2類ポリシー群の開発計画・技術要件・バージョン・リリース方針整理
...
- `AGENTS.md` を読んでいる。
- 2類ポリシー群のうち、作業責務に対応する責務別ポリシーを読んでいる。
...
| v.1.2 | Phase 1 | v.1.2 | Phase 1 実装完了、Docker 上の Deno ランタイム環境、主要検証結果、Phase 2 着手条件を反映 |
| v.1.3 | Phase 2 | v.2.3 | 計画バージョンとフェーズ基準バージョンを分離し、Phase 2 着手前仕様、実装順序、必須検証を追加 |
| v.1.4 | 全フェーズ共通 | - | 開発計画は3類マスター仕様書に基づいて策定される原則を明記 |
| v.1.5 | Phase 2 | v.2.3 | WYSIWYG エディターの実装時期、採用Phase、連携範囲を未定としてPhase 2計画から分離 |
| v.1.6 | 全フェーズ共通 | - | GitHub 互換方針の対象となる機能体系、用語、主要ワークフローを反映 |
| v.1.7 | 全フェーズ共通 | - | UI、画面デザイン、画面レイアウト、視覚表現を GitHub 互換対象外として明記 |
| v.1.8 | 全フェーズ共通 | - | 機能互換は GitHub 互換方針とする表現へ統一 |
| v.1.9 | 全フェーズ共通 | - | オープンソース Git プロバイダーは機能面の参考対象であり互換方針ではないことを明記 |
| v.1.10 | 全フェーズ共通 | - | 基本的な機能互換は GitHub 互換基準、OSS Git プロバイダーはサブの機能互換インスパイア対象と明記 |
| v.1.11 | 全フェーズ共通 | - | マスター実装機能候補リストとの関係を明記 |
| v.1.12 | 全フェーズ共通 | - | TypeScript は6系の最新安定版を採用方針とすることを明記 |
| v.1.13 | 全フェーズ共通 | - | TypeScript 以外の採用技術も最新安定版を採用方針とすることを明記 |
| v.1.14 | 全フェーズ共通 | - | 承認済み固定採用バージョンを明記 |
| v.1.15 | 全フェーズ共通 | - | マスター実装機能候補リストとマスター仕様書の役割重複を整理 |
| v.1.16 | 全フェーズ共通 | - | 2類責務別ポリシー群への参照整合を反映 |
| v.1.17 | 全フェーズ共通 | - | 開発計画の改訂単位をフェーズ単位に統一 |
