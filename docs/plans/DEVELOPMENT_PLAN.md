...
Phase 7 の初回安定版リリース `v.1.8` は完了済みの履歴である。Phase 8 は長期運用準備とマスター仕様完成を扱うが、`deno.json` の内部バージョン更新は実装設定変更に該当するため、別途ユーザー承認を得るまで現行フェーズ基準バージョンは `v.1.8` を維持する。Phase 8 以降の計画判断では、3類マスター仕様書の現行正本仕様、2類ポリシー、本書の現行フェーズ定義を基準にする。
...
Phase 8 の基準バージョンは `v.1.9` とする。Phase 7 で初回安定版リリース `v.1.8` が完了済みであるため、Phase 8 は安定版系列の長期運用準備フェーズとして扱う。
...
- 長期運用準備
- マスター仕様完成
- 3類マスター仕様書の現行正本仕様、履歴、保留候補、対象外範囲の分離
- 移行性確認
- 監査、保守、運用手順の整理
- 本番サーバ環境向けの標準デプロイ自動化、バックアップ、検証、ロールバック手順の具体化
- Deno single binary 正本成果物、Docker 運用選択肢、host filesystem data 領域、shell script + SSH 補助による標準デプロイ実行方式、`gh` と systemd timer の補助採用範囲の具体化
- Adlaire Deploy は仕様未定の将来候補として保留し、当面は Adlaire Git Repository 本体の長期運用準備、標準デプロイ雛形、検証導線、整合性向上を優先する
...
Adlaire Deploy は、Phase 8 では仕様確定対象ではなく、実装計画から除外する。検討再開時は、必要性、責務、連携境界、検証範囲、既存 shell script からの移行範囲を再提案し、別途ユーザー承認を得る。
