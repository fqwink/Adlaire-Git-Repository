...
安定版リリースを表す `Major` 更新は、7系フェーズをデフォルトとし、5系・9系フェーズは補助的にケースバイケースで扱う。
...
| Phase 5 | v.0.6 | 未着手 | 補助的リリース判定フェーズ、デザイン関連の改良・改修。ケースバイケースで安定版対象。安定版承認時のみ `v.1.6` へ進める |
| Phase 6 | v.0.7 | 未着手 | 補助的リリース判定後の機能拡張、運用改善、検証 |
...
Phase 5 の基準バージョンは `v.0.6` とする。Phase 5 を補助的な安定版リリースフェーズとして承認する場合のみ、安定版系列 `v.1.6` へ進める。
