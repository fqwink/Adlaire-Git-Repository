...
Phase 7 の初回安定版リリース `v.1.8` は完了済みの履歴である。Phase 8 は DB 仕様完成フェーズとして libSQL 標準化を扱い、Phase 8.1 は本体整合性、Phase 8.5 はシステム分割、Phase 8.7 は安定化として完了済みである。Phase 9 は Phase 8 系成果のバグ修正ゼロ化、安定版判定、リリース準備として完了済みの履歴である。Phase 10 は現行リリース配置を GitHub Releases に整合しつつ、Adlaire Pipeline をリリース、自動実行、成果物管理、デプロイ反映、実行履歴・監査を担う付随システム候補として定義した履歴である。Phase 11 の現行フェーズ基準バージョンは安定版 `v.2.10` とする。Phase 11 は Go 移行準備、ヘッドレスアーキテクチャ設計思想、Adlaire 公式 SDK 接続一本化、公開 API 直接利用禁止、SDK マスター仕様書、SDK の TypeScript 実装 / JavaScript 生成 / `sdk/` 配置 / 独立リリース方針、Docker の Adlaire Pipeline 経由方針を整理し、3類マスター仕様書を完成版として整合する。さらに、Adlaire Portal System のバージョンポリシーを `v.` 表記維持で適用し、開発版 `v.0.N` と安定版 `v.X.Y` を分離する。Phase 11 以降の計画判断では、3類マスター仕様書の現行正本仕様、2類ポリシー、本書の現行フェーズ定義を基準にする。
...
- バージョン表記は、開発版 `v.0.N` と安定版 `v.X.Y` に分ける。`N` は開発版の累積変更番号、`X` は安定版リリースの通し番号、`Y` は切り出し時点の開発版 `N` とする。いずれもリセットしない。
...
バージョン番号は累積フェーズ番号とは別に管理する。

開発版バージョンは `v.0.N` として管理し、変更ごとに `N` を積み上げる。

安定版バージョンは `v.X.Y` として管理し、`X` は安定版リリースの通し番号、`Y` は切り出し時点の開発版 `N` とする。安定版リリースは7系フェーズをデフォルトとする。ただし、Phase 9 はユーザー承認に基づく安定版判定フェーズとして扱う。後続9系フェーズは自動的な安定版判定フェーズまたは補助的なリリース判定フェーズとして扱わず、安定版判定を行う場合はマスター開発計画と2類ポリシーに明記し、ユーザー承認を得る。5系フェーズと6系フェーズでは安定版バージョンを切り出さない。
...
上記は各フェーズの基準バージョンである。開発版 `v.0.N` と安定版 `v.X.Y` は分離して扱う。

フェーズ内でドキュメント更新、バグ修正、検証追加、仕様整理が発生する場合は、開発版 `N` を累積で進める。次フェーズへ進む場合も、`N` は直前の正式記録バージョンより大きい累積値にする。
...
安定版リリース時も、開発版 `N` はリセットしない。フェーズ番号、開発版 `N`、安定版リリース通し番号 `X`、切り出し時点番号 `Y` のいずれも過去の値へ戻してはならない。
...
| v.0.49 | 全フェーズ共通 / Phase 11 | v.2.10 | Adlaire Portal System のバージョンポリシーを `v.` 表記維持で適用し、開発版 `v.0.N` と安定版 `v.X.Y` を分離。旧方針で記録済みの `v.1.x` / `v.2.x` 開発・文書改訂履歴は移行前履歴として保持 |
