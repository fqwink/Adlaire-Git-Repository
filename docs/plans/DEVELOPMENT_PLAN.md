...
- 機能互換は GitHub 互換方針とする。ただし、UI、画面デザイン、画面レイアウト、視覚表現は GitHub 互換の対象外とする。互換範囲は3類マスター仕様書と承認済みフェーズ計画に定義された範囲に限る。
- オープンソースの Git プロバイダーは機能面の参考・インスパイア対象として扱う。ただし、互換方針の対象ではなく、参考にした機能は3類マスター仕様書へ再定義し、ユーザー承認を得てから実装対象に含める。
