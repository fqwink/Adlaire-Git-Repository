...
将来的に Adlaire Pipeline をリリース成果物、release notes、checksum、manifest、release 履歴の正本にする場合は、リリース配置先の変更として扱い、1類ルールブック、2類ポリシー、3類マスター仕様書、タスク管理ファイルを改訂し、ユーザー承認を得る。

Linux 向け Go single binary を安定版リリース成果物とする場合は、ARM64 と x86_64 の2種類を標準配置対象とする。

Adlaire 公式 SDK は、Adlaire Git Repository 本体へ同梱せず、独立リリース対象とする。SDK のリポジトリ分離は現時点では未定とし、当面は現行リポジトリ内の `sdk/` で管理する。SDK 配布方式は現行リポジトリで扱い、リポジトリ分離時は分離先リポジトリで扱う。SDK リリース開始はタスク管理ファイルと本ポリシーに従い、リリース作業ごとにユーザー承認を得る。SDK のリリースは Adlaire Git Repository 本体の安定版リリースを自動的に意味せず、本体の安定版リリースも SDK のリリースを自動的に意味しない。
...
自動化する場合も、対象バージョン、対象タスク、配置先、成果物、検証方法、自動実行範囲を事前に提示し、ユーザー承認を得なければならない。
