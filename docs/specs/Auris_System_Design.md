...
本プロジェクトの機能範囲、技術方針、セキュリティ方針、現行正本仕様、運用方針は、本書を基準として判断する。

本書における「マスター仕様完成」とは、現行方針として採用済みの仕様、保留中の仕様、対象外の仕様、過去フェーズの履歴を分離し、実装判断に必要な正本仕様を本書と個別3類マスター仕様書から追跡できる状態を指す。

Phase 7 の初回安定版リリース `v.1.8` は履歴として保持する。今後の現行仕様判断では、本書の現在仕様、個別3類マスター仕様書、タスク管理ファイル、2類ポリシーを正本として扱う。
...
今後の作業対象は、タスク管理ファイルである `docs/tasks/TASK_MANAGEMENT.md` で管理する。

未策定、未改訂、未実装タスクは `docs/tasks/TASK_MANAGEMENT.md` で管理する。ただし、同ファイルは実装承認ではない。タスクを実施対象へ確定する場合は、本書、個別3類マスター仕様書、タスク管理ファイルへ反映し、ユーザー承認を得る。

タスク管理ファイルは、未策定、未改訂、未実装タスクを管理する。正式仕様、実装対象、対象外、完了条件、検証範囲は、本書、個別3類マスター仕様書、タスク管理ファイルで管理する。
...
本書は、システム全体の正本仕様を定義する。個別機能の詳細、SDK の詳細、フェーズ別の実施順序、候補分類を本書へ過度に集約してはならない。

仕様判断では、まず本書で全体方針と採用・未確定・対象外の境界を確認し、次に個別3類マスター仕様書で責務範囲の詳細を確認する。作業対象はタスク管理ファイルで確認する。

本書に書かれている現行仕様は、実装承認、リリース承認、外部依存採用承認を意味しない。実装、依存関係、固定バージョン、リリース、デプロイを行う場合は、1類ルールブックと2類ポリシーに従い、別途ユーザー承認を得る。

### 2.1 仕様判断フロー

3類マスター仕様書を読む場合は、以下の順で判断する。

| 順序 | 確認対象 | 判断内容 |
|---:|---|---|
| 1 | 本書の現行正本仕様 | 全体方針として採用済みか、未確定か、対象外か |
| 2 | 個別3類マスター仕様書 | 本体またはSDKの責務範囲に属する詳細か |
| 3 | 2類ポリシー | 技術、リリース、デプロイ、テスト、ライセンスの条件に違反しないか |
| 4 | タスク管理ファイル | 未策定、未改訂、未実装タスクとして管理されているか |
| 6 | README / 実装 / 検証導線 | 利用者向け説明、実装、検証が正本仕様と矛盾しないか |

上記のいずれかで未確定、対象外、保留、未承認に該当する場合、その項目は実装、リリース、デプロイ、依存関係採用へ進めてはならない。

### 2.2 マスター仕様判断チェック

3類マスター仕様書を改善または参照する場合は、最低限以下を確認する。

| 確認項目 | 判定基準 |
|---|---|
| 正本性 | 現行正本仕様、履歴、未確定、保留、対象外が分離されている |
| 責務 | 全体仕様、本体仕様、SDK仕様のどれに属するか説明できる |
| 実装可否 | タスク管理ファイルの対象タスク、対象外、検証範囲へ追跡できる |
| 技術条件 | Go、libSQL、Docker、SDK、Node.js / npm 禁止方針と矛盾しない |
| 承認条件 | 実装、依存採用、固定バージョン、リリース、デプロイの別承認要否を説明できる |
| 整合性 | 2類ポリシー、個別3類マスター仕様書、README、検証導線と矛盾しない |

上記を満たさない記述は、現行正本仕様として扱わない。必要に応じて個別3類マスター仕様書またはタスク管理ファイルへ責務を移し、重複または古い表記を補正する。

### 2.3 3類マスター仕様書の改善基準

3類マスター仕様書を改善する場合は、3本の仕様書を同じ粒度へ機械的に揃えることを目的にしてはならない。各仕様書は、自分の責務範囲だけを正本として詳細化する。
...
- 本書だけで全体方針と判断順序を説明できる。
- 本体仕様書だけで本体が提供する公開契約と内部境界を説明できる。
- SDK仕様書だけでSDKが依存してよい契約と依存してはならない内部構造を説明できる。
- 未確定事項を、現行正本仕様、対象外、履歴へ読み替えていない。
- 同じ詳細条件を複数の3類マスター仕様書へ重複管理していない。
...
Go 標準ライブラリを優先する。Go module、JSR レジストリの公開ライブラリ、その他外部ライブラリは、必要最小限の外部ライブラリ例外採用として扱い、採用前に必要性、対象、固定バージョン、検証方法を提示し、ユーザー承認を得る。
...
採用バージョンは、各技術の最新の安定版を基本方針とする。Go、libSQL、既存データ移行元確認用 SQLite、Git、Go で利用する外部コマンド、例外採用する外部ライブラリは、採用または更新の時点で公式情報を確認し、最新の安定版を採用候補とする。Go の固定採用バージョンは `v1.27.1` とする。
...
| ドキュメント | 正本として管理する範囲 |
|---|---|
| 本書 | システム全体の目的、上位方針、技術方針、現行正本仕様、完成判定、旧 Phase 履歴の仕様境界 |
...
3類マスター仕様書を改善する場合は、以下を満たす必要がある。

- 本書は全体方針を明確化し、個別仕様書の詳細を重複管理しない。
- 個別仕様書は自分の責務範囲を詳細化し、本書の全体方針へ違反しない。
- タスク管理ファイルは仕様を決める文書ではなく、未策定、未改訂、未実装タスクを管理する文書として維持する。
- タスク管理ファイルは、未策定、未改訂、未実装タスクの管理に限定し、正式仕様や実装承認として扱わない。
...
仕様項目は、以下のいずれかに分類して扱う。

| 分類 | 扱い | 実装可否 |
|---|---|---|
| 現行正本仕様 | 採用済みの仕様。個別3類マスター仕様書とタスク管理ファイルへ追跡できる必要がある | 承認済みタスクと実装承認がある場合のみ可 |
| 未確定 | 採用可否、実装時期、固定バージョン、配置、詳細API等が未決定 | 実装不可 |
| 保留 | 将来候補または再検討対象 | 保留解除とユーザー承認まで実装不可 |
| 対象外 | 現行仕様として実施しない範囲 | 方針変更とユーザー承認まで実装不可 |
| 履歴 | 過去フェーズ、旧実装、過去方針の記録 | 現行仕様として実装不可 |

分類が曖昧な項目は、現行正本仕様として扱わない。必ず分類を確定し、必要な正本ドキュメントへ反映してから判断する。

現行仕様の読み替えは禁止する。特に、過去フェーズの履歴、旧 Deno + TypeScript 実装資産、旧 SQLite 標準運用、旧 Docker 方針、Adlaire Pipeline 検討事項を、現行標準仕様として扱ってはならない。

現行仕様を変更する場合は、変更対象が本書だけで完結するか、個別3類マスター仕様書、2類ポリシー、タスク管理ファイル、README、実装、検証導線へ波及するかを確認し、波及する場合は同一変更範囲で整合させる。
...
Go single binary、release notes、checksum、manifest は GitHub Releases 側へ配置する。リポジトリ内に、変更履歴、リリース履歴、release notes 元資料、リリース配置記録、リリース用 manifest、リリース用 checksum を履歴ファイルとして保持してはならない。
...
- 1類ルールブック、2類ポリシー、3類マスター仕様書、タスク管理ファイルの役割が分離されている。
...
- 本書、Adlaire Git Repository 本体仕様、Adlaire 公式 SDK 仕様の3本を読めば、全体方針、本体責務、SDK責務、未確定範囲、対象外範囲を分けて説明できる。
- 現行正本仕様、未確定、保留、対象外、履歴の分類を、実装可否と分けて説明できる。
- 仕様判断フローに従い、どの文書を根拠に判断したかを追跡できる。
- マスター仕様判断チェックにより、仕様記述、実装可否、承認要否、整合対象を同時に確認できる。
- 3類マスター仕様書の改善基準に従い、本書、本体仕様書、SDK仕様書の責務分担を説明できる。
- SDK 詳細 API、Adlaire Pipeline、Turso Cloud 等の未確定事項を、承認なしに現行実装対象へ読み替えていない。

---

## 8. 旧 Phase 履歴

本章は過去の Phase 履歴であり、現行仕様または今後の作業管理ではない。今後の作業対象、対象外範囲、検証範囲、完了条件は `docs/tasks/TASK_MANAGEMENT.md` をタスク管理ファイルとして参照する。本章と現行正本仕様が矛盾する場合は、現行正本仕様を正とする。

フェーズ番号は累積方式とし、安定版リリース後もリセットしない。バージョンは開発版 `v.0.N` と安定版 `v.X.Y` に分ける。`N` は開発版の累積変更番号、`X` は安定版リリースの通し番号、`Y` は切り出し時点の開発版 `N` として扱う。
...
今後の変更作業では、作業完了前にドキュメント等の整合性向上を必ず行う。リポジトリ整合性が取れていない状態で、次のタスクに進んではならない。
...
- DB 関連テストと検証導線の整理
- DB 方針に伴う3類マスター仕様書、2類ポリシー、タスク管理ファイル、README の整合性確認
...
- 1類ルールブック、2類ポリシー、3類マスター仕様書、タスク管理ファイル、README の DB 方針が矛盾していない。
...
- 1類ルールブック、2類ポリシー、3類マスター仕様書、タスク管理ファイル、README、実装、テスト、検証導線、バージョン表記、Pull Request 説明の整合性が確認されている。
...
- Go single binary の成果物、必要な checksum、manifest、release notes の扱いを説明できる。
...
- 3類マスター仕様書、2類リリースポリシー、2類デプロイポリシー、タスク管理ファイル、README の記述が矛盾していない。
...
安定版リリースは、ユーザーが明示承認したリリースタスクに限り実施する。

リリースするには、以下を満たす必要がある。

- 安定版リリースタスクとして明示承認されている。
- 3類マスター仕様書の対象範囲を満たしている。
- 主要検証が完了している。
- 既知バグが残っていない。
- セキュリティ要件を満たしている。
- バージョン表記が開発版 `v.0.N` または安定版 `v.X.Y` 形式である。
