...
libSQL の実採用タイミング、driver 実装、運用形態は、Phase 計画とユーザー承認に基づいて決定する。

SQLite 互換DBの内製化検討理由は、libSQL の採用検討理由と同じである。機能面も libSQL と同等の SQLite 互換DBとして扱い、SQLite 互換、移行容易性、将来の同期・分散構成への拡張余地、DB抽象化設計との相性を評価するための長期研究候補として保持する。

ただし、内製SQLite互換DBは Phase 1 の実装対象に含めない。内製化を進める場合は、SQLite互換性、ACID、WAL、ロック制御、クラッシュ復旧、migration、バックアップ、検証コストを別途マスター仕様書で定義し、ユーザー承認を得る。
...
- SQLite 標準互換 SQL の維持
- 将来の `sqlite` / `libsql` / `internal-sqlite-compatible` driver 差し替え境界の提供
