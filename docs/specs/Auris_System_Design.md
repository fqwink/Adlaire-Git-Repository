...
libSQL は、SQLite からの将来移行候補として扱う。ただし、libSQL の採用は外部ライブラリまたは外部基盤の例外採用に該当するため、採用時はユーザー承認を必須とする。

Turso Cloud 等の libSQL 系クラウドDBホスティングを採用するかどうかは未定とする。クラウドDBホスティングを検討する場合は、セルフホスト方針、クローズドライセンス、データ管理責任、認証情報管理、運用コスト、障害時の復旧方針を3類マスター仕様書に追記してから判断する。
...
- SQLite 標準互換 SQL の維持
- 将来の `sqlite` / `libsql` / `turso` 等の driver 差し替え境界の提供

Phase 1 では driver 実装は SQLite のみとする。ただし、インターフェース、設定名、ディレクトリ構造は libSQL への移行余地を前提に固定する。
