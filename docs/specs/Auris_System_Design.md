...
仕様判断では、まず本書で全体方針と採用・未確定・対象外の境界を確認し、次に個別3類マスター仕様書で責務範囲の詳細を確認する。フェーズ実施可否はマスター開発計画で確認し、候補の優先理由はマスター実装機能候補リストで確認する。
...
| 4 | マスター開発計画 | 対象フェーズ、検証範囲、完了条件に含まれるか |
| 5 | マスター実装機能候補リスト | 候補分類と優先理由に留まるか、正式仕様へ昇格済みか |
...
| 実装可否 | マスター開発計画の対象フェーズ、対象外、検証範囲へ追跡できる |
...
上記を満たさない記述は、現行正本仕様として扱わない。必要に応じて個別3類マスター仕様書またはマスター開発計画へ責務を移し、重複または古い表記を補正する。
...
- マスター開発計画は仕様を決める文書ではなく、仕様をフェーズ、検証範囲、完了条件へ落とし込む文書として維持する。
...
| 分類 | 扱い | 実装可否 |
|---|---|---|
| 現行正本仕様 | 採用済みの仕様。個別3類マスター仕様書とマスター開発計画へ追跡できる必要がある | 承認済みフェーズ計画と実装承認がある場合のみ可 |
...
現行仕様を変更する場合は、変更対象が本書だけで完結するか、個別3類マスター仕様書、2類ポリシー、マスター開発計画、README、実装、検証導線へ波及するかを確認し、波及する場合は同一変更範囲で整合させる。
