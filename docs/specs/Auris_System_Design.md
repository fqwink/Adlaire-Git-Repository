...
- 候補リストは候補分類と理由に限定し、正式仕様や実装承認として扱わない。
...
- Go 固定採用バージョン、SDK 詳細 API、Adlaire Pipeline、Turso Cloud 等の未確定事項を、承認なしに現行実装対象へ読み替えていない。

---

## 8. 実装フェーズ

過去の Phase 表記は履歴として参照する。今後の作業対象、対象外範囲、検証範囲、完了条件は `docs/tasks/TASK_MANAGEMENT.md` をタスク管理ファイルとして参照する。
