...
本書における完成済み仕様は、現行正本仕様、責務境界、旧 Phase 履歴、対象外範囲、保留候補、検証方針を追跡できる状態を指す。

完成済み仕様は、実装済みコードの存在を意味しない。特に Go 本体実装、Go module 例外採用、内製 libSQL driver の実装詳細は、タスク管理ファイルと別途実装承認がある場合にのみ実装対象へ進める。
...
Phase 2 最小実装では、Wiki page の過去本文履歴、添付ファイル、ページ間リンク解決、Markdown レンダリングは対象外とする。API は JSON として本文を返すため、HTML レンダリング時の XSS 対策は、Web UI 実装時に別途検証対象とする。
...
正式バージョン表記は、開発版 `v.0.N` と安定版 `v.X.Y` に分ける。`deno.json` に互換性上 `X.Y.Patch` 形式を記載する場合は、正式表記に対応する内部表記として扱う。

開発版では正式表記を `v.0.N` とし、`deno.json` 側では `0.N.0` に対応させる。安定版では正式表記を `v.X.Y` とし、`deno.json` 側では `X.Y.0` に対応させる。たとえば `2.10.0` は正式表記 `v.2.10` に対応する。
...
# 具体的な build command は Go 実装構成の承認後に確定する。
...
ローカルに Go が存在しない場合、実行系検証はローカルで完了扱いにしない。この場合は、Go `v1.27.1` を満たす VPS または承認済み検証サーバで、Go 検証導線、内製検証スクリプト、`/health`、主要workflow確認を実施する。Docker を用いる検証、テスト、ビルド、binary 生成は、Adlaire Pipeline 経由の対象として扱う。
