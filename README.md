...
`docs/specs/Adlaire_Deploy_Specification.md` は、Adlaire Git Repository の公式付随システムである Adlaire Deploy の個別マスター仕様書である。Adlaire Deploy は本体へ統合せず、同一リポジトリ内に同居し、デプロイ、検証、バックアップ、ロールバック、deploy manifest 記録を扱う。
