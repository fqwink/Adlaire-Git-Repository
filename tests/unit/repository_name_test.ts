...
Deno.test("repository names preserve valid external repository identifiers", () => {
  ...
  assertEquals(
    createRepositoryId("team-alpha", "service_api"),
    "team-alpha/service_api",
  );
});

Deno.test("repository names reject path traversal before Git or database access", () => {
  assertRejects(
    () => validateRepositoryName("../secret", "name"),
    "alphanumeric",
  );
  assertRejects(
    () => validateRepositoryName("team/secret", "owner"),
    "alphanumeric",
  );
  assertRejects(
    () => validateRepositoryName("repo.git", "name"),
    ".git suffix",
  );
});
