...
Deno.test("git receive-pack requests are treated as write operations", () => {
  const refs = new Request("http://localhost/git/alice/repo.git/info/refs?service=git-receive-pack");
  const pack = new Request("http://localhost/git/alice/repo.git/git-receive-pack", { method: "POST" });
...
Deno.test("git upload-pack requests are treated as read operations", () => {
  const refs = new Request("http://localhost/git/alice/repo.git/info/refs?service=git-upload-pack");
  const pack = new Request("http://localhost/git/alice/repo.git/git-upload-pack", { method: "POST" });
